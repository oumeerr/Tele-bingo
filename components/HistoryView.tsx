            ...
            <div key={i} className="bg-white rounded-[2.5rem] border border-hb-border overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="p-5 flex items-center justify-between bg-hb-bg/40 border-b border-hb-border">
                <div className="flex flex-col">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-[13px] font-black text-white tracking-tight">{h.id}</span>
                    <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-md uppercase border ${h.mode === 'mini' ? 'bg-hb-gold/10 text-hb-gold border-hb-gold/20' : 'bg-hb-gold/20 text-hb-gold border-hb-gold/30'}`}>
                      {h.mode}
                    </span>
                  </div>
                ...
                <div className="text-right">
                  ...
                  <span className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter ${h.status === 'won' ? 'bg-hb-emerald/10 text-hb-emerald' : 'bg-red-500/10 text-red-500'}`}>
                    {h.status}
                  </span>
                </div>
              </div>
              ...
              <div className="p-5 grid grid-cols-12 gap-6 bg-black">
                ...
                <div className="col-span-7">
                  <div className="flex items-center justify-between mb-3 px-1">
                    <span className="text-[9px] font-black text-hb-gold uppercase tracking-widest italic">Match Card #{h.cardId}</span>
                  </div>
                  <div className={`grid ${h.mode === 'mini' ? 'grid-cols-3' : 'grid-cols-5'} gap-1 bg-[#0A0A0A] p-3 rounded-2xl border border-hb-border shadow-inner`}>
                ...
                <div className="col-span-5 flex flex-col">
                  <div className="flex items-center justify-between mb-3 px-1">
                    <span className="text-[9px] font-black text-hb-gold uppercase tracking-widest italic">Call Log</span>
                  </div>
                  <div className="flex-1 bg-[#121212] border border-hb-border/50 rounded-2xl overflow-hidden flex flex-col max-h-[160px]">
                    <div className="flex-1 overflow-y-auto p-2 space-y-1.5 no-scrollbar">
                      {h.calledNumbers.map((num, idx) => {
                        ...
                        return (
                          ...
                          >
                            <span className={`w-5 h-5 shrink-0 rounded-lg flex items-center justify-center text-[8px] font-black border ${isWinningNum ? 'bg-hb-blueblack/20 border-hb-blueblack/10 text-hb-blueblack' : 'bg-black border-hb-border text-hb-muted'}`}>
                              {idx + 1}
                            </span>
                    ...
                    <div className="p-2 bg-black border-t border-hb-border/50 text-center">
                      <span className="text-[8px] font-black text-hb-gold/40 uppercase tracking-widest italic">End of Session</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="px-5 py-4 flex items-center justify-between bg-[#050505] border-t border-hb-border/50">
                <div className="flex items-center gap-4">
                  <div className="flex flex-col">
                    <span className="text-[8px] font-black text-hb-muted uppercase tracking-widest">Players</span>
                    <span className="text-[11px] font-black text-white">{h.players}</span>
                  </div>
                  <div className="w-px h-6 bg-hb-border"></div>
                  <div className="flex flex-col">
                    <span className="text-[8px] font-black text-hb-muted uppercase tracking-widest">Stake</span>
                    <span className="text-[11px] font-black text-white">{h.stake} ETB</span>
                  </div>
                </div>
                {h.status === 'won' && (
